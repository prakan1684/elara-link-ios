...
@property (strong, nonatomic) UIButton *elaraButton;
...
- (void)loadView
{
    ...
    self.elaraButton = [[UIButton alloc] init];
    [self.elaraButton setTitle:@"Elara" forState:UIControlStateNormal];
    [self.elaraButton setTitleColor:CONTROL_GRAY_COLOR forState:UIControlStateNormal];
    self.elaraButton.titleLabel.font = [UIFont systemFontOfSize:13 weight:UIFontWeightSemibold];
    self.elaraButton.translatesAutoresizingMaskIntoConstraints = NO;
    [self.view addSubview:self.elaraButton];
    self.elaraButton.hidden = YES;
    ...
    [self.elaraButton addTarget:self action:@selector(elaraButtonTapped:) forControlEvents:UIControlEventTouchUpInside];
}
...
- (void)updateViewConstraints
{
    if (!self.didSetConstraints)
    {
        ...
        NSDictionary *views = @{@"styleButton": self.styleButton, @"wordScrollView": self.wordScrollView, @"elaraButton": self.elaraButton, @"moreButton": self.moreButton, @"rulerView": self.rulerView};

        [self.view addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|[styleButton][wordScrollView][elaraButton][moreButton]|" options:0 metrics:nil views:views]];
        ...
        [self.view addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|[elaraButton]|" options:0 metrics:nil views:views]];
...
{
    BOOL hidden = (delegate == nil);
    self.moreButton.hidden = hidden;
    self.elaraButton.hidden = hidden;
    _delegate = delegate;
}
...
- (IBAction)elaraButtonTapped:(UIButton *)sender
{
    if (self.fadeOutTimer)
    {
        [self.fadeOutTimer invalidate];
    ...
    if (self.delegate && !self.elaraButton.hidden && [self.delegate respondsToSelector:@selector(smartGuideViewController:didTapOnElaraButton:forBlock:)])
        [self.delegate smartGuideViewController:self didTapOnElaraButton:sender forBlock:self.block];
}
